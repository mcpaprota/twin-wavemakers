...
R = 1; % mode number (integer values generate resonance)
sigma = sqrt(g*R*pi/l*tanh(R*pi/l*d)); % wavemaker frequency (rad/s)
Tdt = 1000; % number of time steps per wave period
I = 20; % number of eigenvalues
...
% analytical solution
[kappaA, eta_hatA, phi_hatA] = ...
    wavemakerAnalytical(l,d,chi_hat,sigma,t,I,g);
% numerical solution
[kappaN, eta_hatN, phi_hatN] = wavemakerEulerMod(l,d,xi,zeta,dt,I,g);
% plotting solution amplitudes
subplot(2,1,1)
plot([0 kappaA],eta_hatA(end,:),'k',[0 kappaN],eta_hatN(end,:),'b')
xlabel('$\kappa$'), ylabel('$\hat{\eta}$ (m)')
subplot(2,1,2)
plot(kappaA,phi_hatA(end,:),'k',kappaN,phi_hatN(end,:),'b')
